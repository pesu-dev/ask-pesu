...
Only what is deployed is supported: the production Spaces, AskPESU (`askpesu`) and AskPESU DB (`askpesu-db`), both built from `main`. Fixes land on `dev` and reach production at the next promotion.
...
- **The Qdrant collection.** Only AskPESU DB writes to it. A way to write to it, or to read another collection, through either service is in scope.
